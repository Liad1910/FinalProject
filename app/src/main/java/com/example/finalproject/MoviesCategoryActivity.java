...
public class MoviesCategoryActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnClearSearch.setOnClickListener(v -> { etSearch.setText(""); searchQuery = ""; applyFilters(); });
    ...
    private void buildLegacyMovies() {
        allMovies.clear();
        addLegacy("titanic_1997", "Titanic (1997)", Arrays.asList("Romance", "Drama"), "https://www.youtube.com/results?search_query=Titanic+trailer", R.drawable.titanic_poster);
        addLegacy("forrest_gump_1994", "Forrest Gump (1994)", Arrays.asList("Drama", "Comedy"), "https://www.youtube.com/results?search_query=Forrest+Gump+trailer", R.drawable.forrest_gump_poster);
        addLegacy("matrix_1999", "The Matrix (1999)", Arrays.asList("Action", "Sci-Fi"), "https://www.youtube.com/results?search_query=The+Matrix+trailer", R.drawable.matrix_poster);
        addLegacy("oppenheimer_2023", "Oppenheimer (2023)", Arrays.asList("Drama"), "https://www.youtube.com/results?search_query=Oppenheimer+trailer", R.drawable.oppenheimer_poster);
        addLegacy("white_chicks_2004", "White Chicks (2004)", Arrays.asList("Comedy"), "https://www.youtube.com/results?search_query=White+Chicks+trailer", R.drawable.white_chicks_poster);
        addLegacy("up_2009", "Up (2009)", Arrays.asList("Comedy"), "https://www.youtube.com/results?search_query=Up+trailer", R.drawable.up_poster);
        addLegacy("to_all_the_boys_2018", "To All the Boys I've Loved Before (2018)", Arrays.asList("Romance"), "https://www.youtube.com/results?search_query=To+All+the+Boys+trailer", R.drawable.to_all_the_boys_poster);
        addLegacy("the_pianist_2002", "The Pianist (2002)", Arrays.asList("Drama"), "https://www.youtube.com/results?search_query=The+Pianist+trailer", R.drawable.the_pianist_poster);
        addLegacy("it_2017", "IT (2017)", Arrays.asList("Horror"), "https://www.youtube.com/watch?v=FnCdOQsX5kc", R.drawable.it_poster);
        applyFilters();
    }
    ...
    private void addLegacy(String id, String title, List<String> genres, String trailerUrl, int posterResId) {
        ...
        m.id = id; m.title = title; m.genres = genres;
        m.trailerUrl = trailerUrl; m.posterResId = posterResId; m.isUserTitle = false;
        allMovies.add(m);
    }

    private void loadUserMoviesFromFirestore() {
        db.collection("titles").whereEqualTo("type", "movie")
                ...
                .addSnapshotListener((snap, e) -> {
                    ...
                    for (DocumentSnapshot d : snap.getDocuments()) {
                        MovieItem m = new MovieItem();
                        m.id = d.getId(); m.title = d.getString("title");
                        m.posterUrl = d.getString("posterUrl"); m.trailerUrl = d.getString("trailerUrl");
                        m.isUserTitle = true;
    ...
    private void applyFilters() {
        ...
        for (MovieItem m : allMovies) {
            if (!"All".equals(selectedGenre) && (m.genres == null || !m.genres.contains(selectedGenre))) continue;
    ...
    private void showGenreDialog() {
        final String[] genres = {"All", "Action", "Comedy", "Drama", "Romance", "Horror", "Sci-Fi"};
        int preselect = 0;
        for (int i = 0; i < genres.length; i++) if (genres[i].equals(selectedGenre)) { preselect = i; break; }
    ...
    private String mapAiGenreToLegacyGenre(String ai) {
        switch (ai.trim().toLowerCase()) {
            ...
            case "sci_fi": case "sci-fi": case "scifi": return "Sci-Fi";
            default: return "All";
        }
    }
}